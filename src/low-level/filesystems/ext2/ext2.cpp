...
bool Ext2FileSystem::MakeDir(const char *path, mode_t mode)
{
    return false;
}

bool Ext2FileSystem::Rename(const char *path, const char *newPath)
{
    return false;
}

bool Ext2FileSystem::RemoveDir(const char *path)
{
    return false;    
}
...
FileSystemHandle Ext2FileSystem::GetFileHandle(const char *path, uint32_t flags)
{
    if(!initOK || (flags & O_CREAT) == O_CREAT)
    {
        return 0;
    }
